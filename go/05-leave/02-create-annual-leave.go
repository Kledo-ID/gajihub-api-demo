...
//
//	go run 02-create-annual-leave.go
//
...
func main() {
	fmt.Println("=== Tambah Cuti Tahunan ===")

	hasil := apiRequest("POST", "/hr/leaves/annualLeaves", nil, map[string]any{
		"hr_employee_id":        1,                                  // WAJIB: ID karyawan
		"hr_approval_status_id": 1,                                  // WAJIB: 1 = menunggu persetujuan, 2 = langsung disetujui, 3 = ditolak
		"date_request":          tanggal(0),                         // WAJIB: tanggal pengajuan (YYYY-MM-DD)
		"date_leaves":           []string{tanggal(7)},               // WAJIB: daftar tanggal cuti (bisa lebih dari 1)
