...
//
//	go run 02-export-payroll-by-period.go
//
...
func main() {
	fmt.Println("=== Export Gaji per Periode (Excel) ===")

	unduhFile("/hr/payrolls/perPeriod/export/xls", url.Values{
...
// bulanLalu mengembalikan bulan lalu (YYYY-MM).
func bulanLalu() string {
	sekarang := time.Now().In(wib)

	// Mundur 1 hari dari tanggal 1 bulan ini = tanggal terakhir bulan lalu
	return time.Date(sekarang.Year(), sekarang.Month(), 1, 0, 0, 0, 0, wib).AddDate(0, 0, -1).Format("2006-01")
}
