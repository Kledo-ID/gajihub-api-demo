...
//
//	go run 03-salary-components.go
package main
...
// responsKomponen adalah bagian respons yang dipakai demo ini.
type responsKomponen struct {
	Data struct {
		Data []struct {
			ID              int    `json:"id"`
			Name            string `json:"name"`
			HrComponentType struct {
				Name string `json:"name"`
			} `json:"hr_component_type"`
		} `json:"data"`
	} `json:"data"`
}

func main() {
	fmt.Println("=== Daftar Komponen Gaji ===")

	hasil := apiRequest("GET", "/hr/salaryComponents/pagination", url.Values{
		"page":     {"1"},
		"per_page": {"50"},
	}, nil)

	if hasil.Sukses {
		var respons responsKomponen
		if json.Unmarshal(hasil.Isi, &respons) == nil {
			for _, komponen := range respons.Data.Data {
				fmt.Printf("ID %d : %s (%s)\n", komponen.ID, komponen.Name, komponen.HrComponentType.Name)
			}
			fmt.Println()
		}
	}
