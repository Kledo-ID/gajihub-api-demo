...
//
//	go run download-attendance-reports.go                        -> laporan kemarin
//	go run download-attendance-reports.go 2026-09-01 2026-09-30   -> laporan rentang tanggal
//
...
//   - Windows : Task Scheduler (lihat go/README.md)
//   - Linux   : cron, contoh setiap jam 06:00:
//     0 6 * * * cd /lokasi/gajihub-api-demo/go/11-scheduled-download && go run download-attendance-reports.go
//
// Untuk penjadwalan, lebih hemat menjalankan hasil kompilasi daripada "go run":
//
//	go build -o unduh-absensi download-attendance-reports.go
package main
...
func main() {
	fmt.Println("=== Unduh Otomatis Laporan Absensi ===")

	// Tanggal diambil dari perintah terminal. Jika tidak diisi, pakai tanggal kemarin.
	tanggalMulai := tanggal(-1)
	if len(os.Args) > 1 {
		tanggalMulai = os.Args[1]
	}

	tanggalSelesai := tanggalMulai
	if len(os.Args) > 2 {
		tanggalSelesai = os.Args[2]
	}
	...
	unduhFile("/hr/attendances/detail/export/xls", periode)

	// 2. Rekap absensi per karyawan
	unduhFile("/hr/attendances/summary/export/xls", periode)

	// Tambahkan laporan lain di sini jika perlu, contoh:
	// unduhFile("/hr/overtimes/export/xls", periode)   // data lembur
