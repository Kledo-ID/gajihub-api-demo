...
func main() {
	fmt.Println("=== Daftar Lembur ===")

	hasil := apiRequest("GET", "/hr/overtimes/pagination", url.Values{
...
// akhirBulan mengembalikan tanggal terakhir bulan ini (YYYY-MM-DD).
func akhirBulan() string {
	sekarang := time.Now().In(wib)

	// Tanggal 0 bulan berikutnya = tanggal terakhir bulan ini
	return time.Date(sekarang.Year(), sekarang.Month()+1, 0, 0, 0, 0, 0, wib).Format("2006-01-02")
}
