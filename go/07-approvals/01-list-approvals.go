...
// responsPersetujuan adalah bagian respons yang dipakai demo ini.
type responsPersetujuan struct {
	Data struct {
		Data []struct {
			ID             int `json:"id"`
			HrApprovalType struct {
				Name string `json:"name"`
			} `json:"hr_approval_type"`
			HrEmployee struct {
				Name string `json:"name"`
			} `json:"hr_employee"`
		} `json:"data"`
...
func main() {
	fmt.Println("=== Daftar Persetujuan ===")

	hasil := apiRequest("GET", "/hr/approvals/pagination", url.Values{
	...
	if hasil.Sukses {
		var respons responsPersetujuan
		if json.Unmarshal(hasil.Isi, &respons) == nil {
			for _, pengajuan := range respons.Data.Data {
				fmt.Printf("ID %d : %s - %s\n", pengajuan.ID, pengajuan.HrApprovalType.Name, pengajuan.HrEmployee.Name)
			}
			fmt.Println()
		}
	}
