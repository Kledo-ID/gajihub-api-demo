...
// responsAbsensi adalah bagian respons yang dipakai demo ini.
type responsAbsensi struct {
	Data struct {
...
func main() {
	fmt.Println("=== Absensi Harian Semua Karyawan ===")

	hasil := apiRequest("GET", "/hr/attendances/daily/pagination", url.Values{
		"date":     {tanggal(0)}, // tanggal absensi, format YYYY-MM-DD (contoh: hari ini)
		"page":     {"1"},        // halaman ke-
	...
	if hasil.Sukses {
		var respons responsAbsensi
		if json.Unmarshal(hasil.Isi, &respons) == nil {
			halaman := respons.Data

			fmt.Printf("Total karyawan: %d (halaman %d dari %d)\n", halaman.Total, halaman.CurrentPage, halaman.LastPage)
			fmt.Println()
		}
	}
