...
//
//	go run 03-attendance-summary.go
package main
...
func main() {
	fmt.Println("=== Rekap Absensi per Karyawan ===")

	hasil := apiRequest("GET", "/hr/attendances/summary/pagination", url.Values{
		"date_started": {awalBulan()}, // tanggal mulai, format YYYY-MM-DD (contoh: awal bulan ini)
		"date_ended":   {tanggal(0)},  // tanggal selesai (contoh: hari ini)
		"page":         {"1"},
		"per_page":     {"20"},
