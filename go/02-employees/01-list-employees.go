...
// responsKaryawan adalah bagian respons yang dipakai demo ini.
type responsKaryawan struct {
	Data struct {
...
func main() {
	...
	hasil := apiRequest("GET", "/hr/employees/pagination", url.Values{
		"page":     {"1"},
		"per_page": {"20"},
	...
	if hasil.Sukses {
		...
		if json.Unmarshal(hasil.Isi, &respons) == nil {
			...
			for _, karyawan := range halaman.Data {
				fmt.Printf("ID %d : %s - %s\n", karyawan.ID, karyawan.Code, karyawan.Name)
			}
			fmt.Println()
		}
	}
