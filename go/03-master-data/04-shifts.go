...
// responsShift adalah bagian respons yang dipakai demo ini.
type responsShift struct {
	Data []struct {
		ID          int    `json:"id"`
...
func main() {
	fmt.Println("=== Daftar Shift Kerja ===")

	hasil := apiRequest("GET", "/hr/shifts", nil, nil)

	if hasil.Sukses {
		var respons responsShift
		if json.Unmarshal(hasil.Isi, &respons) == nil {
			for _, shift := range respons.Data {
				fmt.Printf("ID %d : %s (%s - %s)\n", shift.ID, shift.Name, shift.TimeStarted, shift.TimeEnded)
			}
			fmt.Println()
		}
	}
