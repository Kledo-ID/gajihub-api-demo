...
// responsOrganisasi adalah bagian respons yang dipakai demo ini.
type responsOrganisasi struct {
	Data []struct {
		ID   int    `json:"id"`
...
func main() {
	fmt.Println("=== Daftar Struktur Organisasi ===")

	hasil := apiRequest("GET", "/hr/orgStructures", url.Values{
		// "is_archive": {"all"}, // not_archive (default) / archive / all
	}, nil)

	if hasil.Sukses {
		var respons responsOrganisasi
		if json.Unmarshal(hasil.Isi, &respons) == nil {
			for _, organisasi := range respons.Data {
				induk := "-"
				if organisasi.ParentID != nil {
					induk = fmt.Sprint(*organisasi.ParentID)
				}
				fmt.Printf("ID %d : %s (induk: %s)\n", organisasi.ID, organisasi.Name, induk)
			}
			fmt.Println()
		}
	}
