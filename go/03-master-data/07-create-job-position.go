...
//
//	go run 07-create-job-position.go
//
...
func main() {
	fmt.Println("=== Tambah Jabatan ===")

	hasil := apiRequest("POST", "/hr/jobPositions", nil, map[string]any{
		"name":      "Staff Gudang " + time.Now().Format("150405"), // WAJIB: nama jabatan (2-45 karakter, tidak boleh sama)
		"parent_id": nil,                                           // opsional: ID jabatan atasan
	})
	...
	if hasil.Sukses {
		...
		if json.Unmarshal(hasil.Isi, &respons) == nil {
			fmt.Println()
			fmt.Printf("ID jabatan baru: %d (pakai ID ini di demo ubah / hapus)\n", respons.Data.ID)
		}
	}
}
