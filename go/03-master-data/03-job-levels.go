...
// responsLevel adalah bagian respons yang dipakai demo ini.
type responsLevel struct {
	Data []struct {
		ID   int    `json:"id"`
...
func main() {
	fmt.Println("=== Daftar Level Jabatan ===")

	hasil := apiRequest("GET", "/hr/jobLevels", nil, nil)

	if hasil.Sukses {
		var respons responsLevel
		if json.Unmarshal(hasil.Isi, &respons) == nil {
			for _, level := range respons.Data {
				fmt.Printf("ID %d : %s\n", level.ID, level.Name)
			}
			fmt.Println()
		}
