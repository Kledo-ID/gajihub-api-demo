...
// responsJabatan adalah bagian respons yang dipakai demo ini.
type responsJabatan struct {
	Data []struct {
		ID   int    `json:"id"`
...
func main() {
	fmt.Println("=== Daftar Jabatan ===")

	hasil := apiRequest("GET", "/hr/jobPositions", nil, nil)

	if hasil.Sukses {
		var respons responsJabatan
		if json.Unmarshal(hasil.Isi, &respons) == nil {
			for _, jabatan := range respons.Data {
				atasan := "-"
				if jabatan.ParentID != nil {
					atasan = fmt.Sprint(*jabatan.ParentID)
				}
				fmt.Printf("ID %d : %s (atasan: %s)\n", jabatan.ID, jabatan.Name, atasan)
			}
			fmt.Println()
		}
	}
