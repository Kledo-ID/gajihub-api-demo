...
console.log('=== Daftar Komponen Gaji ===');

const hasil = await apiRequest('GET', '/hr/salaryComponents/pagination', {
    page: 1,
    per_page: 50,
});
...
if (hasil.sukses) {
    for (const komponen of hasil.data.data.data) {
        console.log(`ID ${komponen.id} : ${komponen.name} (${komponen.hr_component_type.name})`);
    }
    console.log();
}
