 ...
 * lihat folder express/ di sebelah file ini.
 */
...
(async () => {
    ...
    const hasil = await apiRequest('GET', '/authentication/user');

    if (hasil.sukses) {
        const user = hasil.data.data;

        console.log('Token valid.');
        console.log(`Pemilik token: ${user.name} (${user.email})`);
        console.log('Semua request API berjalan dengan hak akses user ini.');
    } else {
        tampilkan(hasil);
    }
})();
