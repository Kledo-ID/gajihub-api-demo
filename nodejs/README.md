...
2. Jalankan lewat terminal:
   ...
   node 01-check-token.js
   ```

Mulailah dari `01-authentication/01-check-token.js` untuk memastikan token sudah benar.
...
> Bagian yang memakai `await` dibungkus dalam fungsi `(async () => { ... })();` agar tetap berjalan
> sebagai file `.js` CommonJS biasa, tanpa konfigurasi tambahan (`type: module`, dsb).
...
Lihat [README utama](../README.md#daftar-demo). Nama file sama untuk semua bahasa, dengan akhiran `.js`.

## Unduh Laporan Otomatis

```bash
# Laporan kemarin
node nodejs/11-scheduled-download/download-attendance-reports.js

# Laporan rentang tanggal tertentu
node nodejs/11-scheduled-download/download-attendance-reports.js 2026-09-01 2026-09-30
```

**Windows (Task Scheduler)** — buat **Basic Task** harian, pilih **Start a program**, lalu isi:

- **Program/script**: `node` (atau `C:\Program Files\nodejs\node.exe`)
- **Add arguments**: `download-attendance-reports.js`
...
```text
0 6 * * * node /lokasi/gajihub-api-demo/nodejs/11-scheduled-download/download-attendance-reports.js >> /var/log/unduh-absensi.log 2>&1
```
