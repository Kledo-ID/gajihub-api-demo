...
console.log('=== Daftar Persetujuan ===');

const hasil = await apiRequest('GET', '/hr/approvals/pagination', {
...
if (hasil.sukses) {
    for (const pengajuan of hasil.data.data.data) {
        console.log(`ID ${pengajuan.id} : ${pengajuan.hr_approval_type.name} - ${pengajuan.hr_employee.name}`);
    }
    console.log();
}
