 ...
 *   node 07-create-job-position.mjs
 *
...
console.log('=== Tambah Jabatan ===');

const jamSekarang = new Date().toTimeString().slice(0, 8).replaceAll(':', ''); // contoh: 142530

const hasil = await apiRequest('POST', '/hr/jobPositions', {
...
if (hasil.sukses) {
    console.log();
    console.log('ID jabatan baru: ' + hasil.data.data.id + ' (pakai ID ini di demo ubah / hapus)');
}
