...
console.log('=== Daftar Struktur Organisasi ===');

const hasil = await apiRequest('GET', '/hr/orgStructures', {
...
if (hasil.sukses) {
    for (const organisasi of hasil.data.data) {
        ...
        console.log(`ID ${organisasi.id} : ${organisasi.name} (induk: ${organisasi.parent_id ?? '-'})`);
    }
    console.log();
}
