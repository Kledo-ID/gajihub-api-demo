...
console.log('=== Daftar Jabatan ===');

const hasil = await apiRequest('GET', '/hr/jobPositions');

if (hasil.sukses) {
    for (const jabatan of hasil.data.data) {
        // parent_id = ID jabatan atasan (kosong jika jabatan paling atas)
        console.log(`ID ${jabatan.id} : ${jabatan.name} (atasan: ${jabatan.parent_id ?? '-'})`);
    }
    console.log();
}
