...
def main():
    print('=== Daftar Persetujuan ===')

    hasil = api_request('GET', '/hr/approvals/pagination', {
    ...
    if hasil['sukses']:
        for pengajuan in hasil['data']['data']['data']:
            print(f"ID {pengajuan['id']} : {pengajuan['hr_approval_type']['name']} - {pengajuan['hr_employee']['name']}")
        print()
