...
echo '=== Daftar Komponen Gaji ===' . PHP_EOL;

$hasil = api_request('GET', '/hr/salaryComponents/pagination', [
    'page'     => 1,
    'per_page' => 50,
]);
...
if ($hasil['sukses']) {
    foreach ($hasil['data']['data']['data'] as $komponen) {
        echo "ID {$komponen['id']} : {$komponen['name']} ({$komponen['hr_component_type']['name']})" . PHP_EOL;
    }
    echo PHP_EOL;
}
