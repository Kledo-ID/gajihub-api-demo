 ...
 * lihat folder laravel/ di sebelah file ini.
 */
...
echo '=== Cek Token ===' . PHP_EOL;

$hasil = api_request('GET', '/authentication/user');

if ($hasil['sukses']) {
    $user = $hasil['data']['data'];

    echo 'Token valid.' . PHP_EOL;
    echo "Pemilik token: {$user['name']} ({$user['email']})" . PHP_EOL;
    echo 'Semua request API berjalan dengan hak akses user ini.' . PHP_EOL;
} else {
    tampilkan($hasil);
}
