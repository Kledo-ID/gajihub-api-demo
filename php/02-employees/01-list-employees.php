...
echo '=== Daftar Karyawan ===' . PHP_EOL;

$hasil = api_request('GET', '/hr/employees/pagination', [
    'page'     => 1,
    'per_page' => 20,
...
if ($hasil['sukses']) {
    ...
    echo "Total karyawan: {$halaman['total']} (halaman {$halaman['current_page']} dari {$halaman['last_page']})" . PHP_EOL;
    foreach ($halaman['data'] as $karyawan) {
        echo "ID {$karyawan['id']} : {$karyawan['code']} - {$karyawan['name']}" . PHP_EOL;
    }
    echo PHP_EOL;
}
