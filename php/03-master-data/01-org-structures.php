...
echo '=== Daftar Struktur Organisasi ===' . PHP_EOL;

$hasil = api_request('GET', '/hr/orgStructures', [
...
if ($hasil['sukses']) {
    foreach ($hasil['data']['data'] as $organisasi) {
        ...
        echo "ID {$organisasi['id']} : {$organisasi['name']} (induk: " . ($organisasi['parent_id'] ?? '-') . ')' . PHP_EOL;
    }
    echo PHP_EOL;
}
